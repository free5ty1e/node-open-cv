...
sudo apt-get -y install build-essential cmake cmake-qt-gui pkg-config libpng12-0 libpng12-dev libpng++-dev libpng3 libpnglite-dev zlib1g-dbg zlib1g zlib1g-dev pngtools libtiff4-dev libtiff4 libtiffxx0c2 libtiff-tools oracle-java7-jdk ant git gcc-4.7 npm node libjpeg-dev libpng-dev libtiff-dev libjasper-dev
...
make -j8
echo Hooray, if no errors are shown above then you might have OpenCV on your Raspberry Pi!
popd
